...
// node_modules/@spyglassmc/mcdoc/lib/runtime/completer/index.js
var completer_exports = {};
__export(completer_exports, {
  getFields: () => getFields,
  getValues: () => getValues
});
function getFields(typeDef, ctx) {
  switch (typeDef.kind) {
    case "union":
    ...
    case "struct":
      return typeDef.fields.flatMap((field) => {
        ...
        if (field.key.kind === "string" || field.key.kind === "literal" && field.key.value.kind === "string" || field.key.kind === "enum" && field.key.enumKind === "string") {
        ...
        if (field.key.kind === "string") {
          return getStringCompletions(field.key, ctx).map((c) => ({ key: c.value, field }));
        }
        return getValues(field.key, ctx).map((c) => ({ key: c.value, field }));
      });
    default:
      return [];
  }
}
function getValues(typeDef, ctx) {
  if (typeDef.kind === "string" || typeDef.kind === "literal" && typeDef.value.kind === "string" || typeDef.kind === "enum" && typeDef.enumKind === "string") {
...
//# sourceMappingURL=chunk-RTAB2HDH.js.map
